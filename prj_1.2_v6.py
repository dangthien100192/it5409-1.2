...
def fourier_noise_filter(image, z_score_threshold=5.5, notch_filter_size=3):
    # FFT of image
    h, w = image.shape
    f = np.fft.fft2(image)
    ...
    # Mask out the DC component (center) to avoid removing valid image data
    center_mask = np.zeros((h, w), dtype=np.uint8)
    cv2.circle(center_mask, (w // 2, h // 2), 5, 1, -1)
    magnitude_spectrum_for_stats = magnitude_spectrum * (1 - center_mask)

    # Detect spikes using z-score
    mean_val = np.mean(magnitude_spectrum_for_stats[magnitude_spectrum_for_stats > 0])
    std_val = np.std(magnitude_spectrum_for_stats[magnitude_spectrum_for_stats > 0])
    z_score = (magnitude_spectrum - mean_val) / std_val

    spikes = (z_score > z_score_threshold) & (1 - center_mask).astype(bool)

    # Dilate spikes to widen notch filter
    dilated_spikes = binary_dilation(spikes, structure=np.ones((notch_filter_size, notch_filter_size)))

    # Create notch filter mask
    notch_filter = np.ones_like(magnitude_spectrum, dtype=np.float32)
    notch_filter[dilated_spikes] = 0
    ...
    return magnitude_spectrum, notch_filter, img_back


def division_noise_filter(image, filter_size=151):
    # 1. Dùng Gaussian filter để tạo nền ước lượng (Illumination Map)
    background = cv2.GaussianBlur(image, (filter_size, filter_size), 0)

    # 2. Image Subtraction: Trừ nền khỏi ảnh gốc
    # Bước này cực kỳ quan trọng: nó đưa các vùng nền về gần mức 0 (đen)
    # cv2.subtract sẽ tự động cắt (clip) các giá trị âm về 0
    diff = cv2.subtract(image, background)

    # 3. Image Division: Chia hiệu số cho ảnh nền để khuếch đại (Gain control)
    # Công thức: (diff / background) * scale
    # Ở vùng tối: background nhỏ -> kết quả phép chia lớn -> hạt gạo mờ được đẩy sáng lên.
    # Ở vùng nền: diff ≈ 0 -> kết quả vẫn bằng 0 -> nền giữ được màu đen.
    # cv2.divide xử lý an toàn việc chia cho 0 và trả về kết quả định dạng uint8
    division = cv2.divide(diff, background, scale=255)

    # 4. Chuẩn hóa lần cuối để đảm bảo dải giá trị nằm trọn trong [0, 255]
    final_img = cv2.normalize(division, None, 0, 255, cv2.NORM_MINMAX)

    # Trả về các bước trung gian để bạn có thể dùng hàm display_step kiểm tra
    return background, division, final_img
...
def count_rice_grains(image_path, output_folder=None, save_steps=False, show_steps=False):
    ...
    # Xử lý fourier
    magnitude_spectrum, notch_filter, gray_fourier = fourier_noise_filter(gray)
    ...
    if save_steps and output_folder:
        save_step_image(output_folder, original_filename, 3, "gray_clahe", gray_clahe)
        save_step_image(output_folder, original_filename, 4, "blur", blur)

    # Subtraction-based Division
    background, division_norm, filtered_img = division_noise_filter(blur)
    
    if save_steps and output_folder:
        save_step_image(output_folder, original_filename, 5, "division_filtered", filtered_img)

    # Threshold Otsu
    _, thresh = cv2.threshold(
        filtered_img, 0, 255, cv2.THRESH_BINARY_INV + cv2.THRESH_OTSU
    )

    white_ratio = np.sum(thresh == 255) / thresh.size
    if white_ratio > 0.70:
        _, thresh = cv2.threshold(
            filtered_img, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU
        )

    if save_steps and output_folder:
        save_step_image(output_folder, original_filename, 6, "thresh", thresh)
    ...
    if save_steps and output_folder:
        save_step_image(output_folder, original_filename, 7, "opening", opening)
        save_step_image(output_folder, original_filename, 8, "closing", closing)

    # Sure BG
    sure_bg = cv2.dilate(closing, kernel, iterations=2)

    if save_steps and output_folder:
        save_step_image(output_folder, original_filename, 9, "sure_bg", sure_bg)
    ...
    if save_steps and output_folder:
        save_step_image(output_folder, original_filename, 10, "distance_transform", dist_transform_norm)
        save_step_image(output_folder, original_filename, 11, "sure_fg", sure_fg)
        save_step_image(output_folder, original_filename, 12, "unknown", unknown)
    ...
    if save_steps and output_folder:
        ...
        save_step_image(output_folder, original_filename, 13, "markers_before_watershed", markers_vis_before)
    ...
    if save_steps and output_folder:
        ...
        save_step_image(output_folder, original_filename, 14, "markers_after_watershed", markers_vis_after)
    ...
    if save_steps and output_folder:
        save_step_image(output_folder, original_filename, 15, "valid_mask", valid_mask)
        save_step_image(output_folder, original_filename, 16, "valid_overlay", valid_overlay)
        save_step_image(output_folder, original_filename, 17, "final_output", output)

    if show_steps:
        cv2.imshow("Original", original)
        cv2.imshow("Gray", gray)
        cv2.imshow("Gray Fourier", gray_fourier)
        cv2.imshow("Gray CLAHE", gray_clahe)
        cv2.imshow("Blur", blur)
        cv2.imshow("Division Filtered", filtered_img)
